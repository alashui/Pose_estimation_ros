...
class TandemObstacleAvoidance
{
  ...
  public:
	...
	TandemObstacleAvoidance(ros::NodeHandle& handle):
		...
		directionsSecondary(FORWARD), last_GROUND_CLOSEY(0), 
		last_GROUND_CLOSEZ(0), last_GROUND_FARY(0), last_GROUND_FARZ(0), 
	...
	void heightRange(const sensor_msgs::PointCloud2& input)
	{
		...
		double GROUND_TOLERANCEFINE, GROUND_TOLERANCEROUGH;
		int HEIGHT_SAMPLES;
		bool HEIGHT_VERBOSE;
		...
		//downsample cloud
		filter_downsample.setInputCloud(cloud);
		...
		*cloud_obstacle = *cloud_downsampled;
		
		//cloud_downsampled_pub.publish(*cloud_obstacle);
		
		//remove points near the ground
		for(auto location =cloud_obstacle->begin(); location < cloud_obstacle->end();)
		{
			...
			{ 
				cloud_obstacle->erase(location);	//delete point near the ground
			}
			else
			{ 
			    location++;
		...
		cloud_obstacle_pub.publish(*cloud_obstacle);
		//crop the cloud
		filter_crop.setInputCloud(cloud_obstacle);
...
int main(int argc, char** argv)
{
	...
	ros::NodeHandle node; //string here is the namespace for parameters
	...
	node.setParam("ground_closez", 0.86); //corresponding z-coordinate for bumper border and modeling the plane
	node.setParam("ground_fary", 0.44); //y-coordinate of a far point on the ground
	node.setParam("ground_farz", 2.23); //corresponding z-coordinate for modeling the plane
